...
def main():
    ...
    index_parser.add_argument("--model_type", type=str, required=True, default="dinov2_vits14_reg",help="Type of model to use")
    index_parser.add_argument("--device", type=str, required=False, default="cuda", help="Device to use for feature extraction (cuda, mps, cpu)")
    index_parser.add_argument("--batch_size", type=int, required=False, default=32, help="Number of images per batch")
    index_parser.add_argument("--overwrite_index", action="store_true", help="Overwrite existing index of same index_path")

    search_parser.add_argument("--query_image", type=str, required=True, help="Path to directory of images to index")
    search_parser.add_argument("--index_path", type=str, required=True, help="Path to saved index")
    search_parser.add_argument("--indexed_images_path", type=str, required=True, help="Path to file containing indexed image paths")
    search_parser.add_argument("--index_type", type=str, required=True, default="L2", help="Type of index to use (currently only using L2)")
    search_parser.add_argument("--top_k", type=int, required=True, default=10, help="top k-Nearest Neighbors to return")
