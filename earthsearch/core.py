...
class FeatureExtractor:
    ...
    def __init__(
        ...
        if model_type == "resnet18":
            ...
            base_model = torch.hub.load("facebookresearch/dinov2", "dinov2_vits14", verbose=False)
            self.embedding_dim = 384
        elif model_type == "dinov2_vits14_reg":
            base_model = torch.hub.load('facebookresearch/dinov2', 'dinov2_vits14_reg', verbose=False)
            self.embedding_dim = 384
        elif model_type == "dinov2_vitb14":
            base_model = torch.hub.load("facebookresearch/dinov2", "dinov2_vitb14", verbose=False)
            self.embedding_dim = 768
        elif model_type == "dinov2_vitb14_reg":
            base_model = torch.hub.load('facebookresearch/dinov2', 'dinov2_vitb14_reg', verbose=False)
            self.embedding_dim = 768
        elif model_type == "dinov2_vitl14":
            base_model = torch.hub.load("facebookresearch/dinov2", "dinov2_vitl14", verbose=False)
            self.embedding_dim = 1024
        elif model_type == "dinov2_vitl14_reg":
            base_model = torch.hub.load('facebookresearch/dinov2', 'dinov2_vitl14_reg', verbose=False)
            self.embedding_dim = 1024
        elif model_type == "dinov2_vitg14":
            base_model = torch.hub.load("facebookresearch/dinov2", "dinov2_vitg14", verbose=False)
            self.embedding_dim = 1536
        elif model_type == "dinov2_vitg14_reg":
            base_model = torch.hub.load('facebookresearch/dinov2', 'dinov2_vitg14_reg', verbose=False)
    ...
    def extract_embedding(
            self,
            src: Union[str, np.ndarray, PIL.Image]
            ) -> np.array:
        """
...
class VectorDatabase:
    ...
    def search(
        ...
        # Search the index
        distances, indices = self.index.search(query_embedding, min(top_k, len(self.image_paths)))
        
        # Get the corresponding image paths
        result_paths = [self.image_paths[idx] for idx in indices[0] if idx >= 0 and idx < len(self.image_paths)]
...
class ImageSimilaritySearch:
    ...
    def index_images(
        ...
        with tqdm(total=len(image_paths), desc="Indexing images", unit="image") as progress_bar:
